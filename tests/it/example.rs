...
fn assert_orc(orc_data: &[u8], jsn_gz_data: &[u8]) {
...
fn orc_to_jsn(orc_data: &[u8]) -> String {
    ...
    let mut lines = data.iter().map(JsonValue::dump).collect::<Vec<_>>();
    ...
    match type_.kind() {
        ...
        _ => {
            ...
            match type_.kind() {
                ...
                Kind::Float => unwrap_deserialization(deserialize_f32_array(&column), &error_msg)
                    .into_iter()
                    .map(|v| match v {
                        Some(n) => JsonValue::Number(Number::from(n)),
                        None => JsonValue::Null,
                    ...
                    .collect(),
                Kind::String => unwrap_deserialization(deserialize_str_array(&column), &error_msg)
                    .into_iter()
                    .map(|v| match v {
...
#[test]
fn test_meta() {
    let orc_data = include_bytes!("../examples/TestOrcFile.metaData.orc");
    let jsn_gz_data: &[u8] = include_bytes!("../examples/expected/TestOrcFile.metaData.jsn.gz");
    assert_orc(orc_data, jsn_gz_data);
}
