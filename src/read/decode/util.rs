use std::io::{self, Error, Read};

pub fn read_byte(reader: &mut dyn Read) -> u8 {
    let mut byte = [0u8];
    let _ = reader.read_exact(&mut byte);

    byte[0]
}

pub fn bytes_to_long_be<R: Read>(r: &mut R, mut n: usize) -> Result<i64, io::Error> {
    let mut out: i64 = 0;

    while n > 0 {
        n -= 1;
        let val = read_byte(r) as i64;
        out |= val << (n * 8) as u64;
    }

    Ok(out)
}

pub fn get_closest_fixed_bits(width: usize) -> usize {
    match width {
        0 => 1,
        1..=24 => width,
        25..=26 => 26,
        27..=28 => 28,
        29..=30 => 30,
        31..=32 => 32,
        33..=40 => 40,
        41..=48 => 48,
        49..=56 => 56,
        _ => 64,
    }
}

pub fn read_ints(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    bit_size: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    let mut bits_left = 0;
    let mut current = 0;

    match bit_size {
        1 => unrolled_unpack_1(buffer, offset, len, r),
        2 => unrolled_unpack_2(buffer, offset, len, r),
        4 => unrolled_unpack_4(buffer, offset, len, r),
        8 => unrolled_unpack_8(buffer, offset, len, r),
        16 => unrolled_unpack_16(buffer, offset, len, r),
        24 => unrolled_unpack_24(buffer, offset, len, r),
        32 => unrolled_unpack_32(buffer, offset, len, r),
        40 => unrolled_unpack_40(buffer, offset, len, r),
        48 => unrolled_unpack_48(buffer, offset, len, r),
        56 => unrolled_unpack_56(buffer, offset, len, r),
        64 => unrolled_unpack_64(buffer, offset, len, r),
        _ => {
            for i in offset..(offset + len) {
                let mut result: i64 = 0;
                let mut bits_left_to_read = bit_size;

                while bits_left_to_read > bits_left {
                    result <<= bits_left;
                    result |= (current & ((1 << bits_left) - 1)) as i64;
                    bits_left_to_read -= bits_left;

                    let mut buf = [0; 1];
                    match r.read_exact(&mut buf) {
                        Ok(_) => current = buf[0] as i32,
                        Err(err) => return Err(err),
                    }

                    bits_left = 8;
                }

                if bits_left_to_read > 0 {
                    result <<= bits_left_to_read;
                    bits_left -= bits_left_to_read;
                    result |= ((current >> bits_left) & ((1 << bits_left_to_read) - 1)) as i64;
                }

                buffer[i] = result;
            }

            Ok(())
        }
    }
}

fn unrolled_unpack_1(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    reader: &mut dyn Read,
) -> Result<(), Error> {
    let num_hops = 8;
    let remainder = len % num_hops;
    let end_offset = offset + len;
    let end_unroll = end_offset - remainder;

    for i in (offset..end_unroll).step_by(num_hops) {
        let byte = read_byte(reader);
        let val = byte as u64;
        buffer[i] = ((val >> 7) & 1) as i64;
        buffer[i + 1] = ((val >> 6) & 1) as i64;
        buffer[i + 2] = ((val >> 5) & 1) as i64;
        buffer[i + 3] = ((val >> 4) & 1) as i64;
        buffer[i + 4] = ((val >> 3) & 1) as i64;
        buffer[i + 5] = ((val >> 2) & 1) as i64;
        buffer[i + 6] = ((val >> 1) & 1) as i64;
        buffer[i + 7] = (val & 1) as i64;
    }

    if remainder > 0 {
        let mut start_shift = 7;
        let val = read_byte(reader) as u64;
        for i in end_unroll..end_offset {
            buffer[i] = ((val >> start_shift) & 1) as i64;
            start_shift -= 1;
        }
    }

    Ok(())
}

fn unrolled_unpack_2(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    reader: &mut dyn Read,
) -> Result<(), Error> {
    let num_hops = 4;
    let remainder = len % num_hops;
    let end_offset = offset + len;
    let end_unroll = end_offset - remainder;

    for i in (offset..end_unroll).step_by(num_hops) {
        let val = read_byte(reader) as u64;
        buffer[i] = ((val >> 6) & 3) as i64;
        buffer[i + 1] = ((val >> 4) & 3) as i64;
        buffer[i + 2] = (val >> 2 & 3) as i64;
        buffer[i + 3] = ((val) & 3) as i64;
    }

    if remainder > 0 {
        let mut start_shift = 6;
        let val = read_byte(reader) as u64;
        for i in end_unroll..end_offset {
            buffer[i] = ((val >> start_shift) & 3) as i64;
            start_shift -= 2;
        }
    }
    Ok(())
}

fn unrolled_unpack_4(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    reader: &mut dyn Read,
) -> Result<(), Error> {
    let num_hops = 2;
    let remainder = len % num_hops;
    let end_offset = offset + len;
    let end_unroll = end_offset - remainder;

    for i in (offset..end_unroll).step_by(num_hops) {
        let val = read_byte(reader) as u64;
        buffer[i] = ((val >> 4) & 15) as i64;
        buffer[i + 1] = (val & 15) as i64;
    }

    if remainder > 0 {
        let mut start_shift = 6;
        let val = read_byte(reader) as u64;
        for i in end_unroll..end_offset {
            buffer[i] = ((val >> start_shift) & 15) as i64;
            start_shift -= 4;
        }
    }
    Ok(())
}

fn unrolled_unpack_8(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 1)
}

fn unrolled_unpack_16(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 2)
}

fn unrolled_unpack_24(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 3)
}

fn unrolled_unpack_32(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 4)
}

fn unrolled_unpack_40(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 5)
}

fn unrolled_unpack_48(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 6)
}

fn unrolled_unpack_56(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 7)
}

fn unrolled_unpack_64(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
) -> Result<(), Error> {
    unrolled_unpack_bytes(buffer, offset, len, r, 8)
}

fn unrolled_unpack_bytes(
    buffer: &mut [i64],
    offset: usize,
    len: usize,
    r: &mut dyn Read,
    num_bytes: usize,
) -> Result<(), Error> {
    let num_hops = 8;
    let remainder = len % num_hops;
    let end_offset = offset + len;
    let end_unroll = end_offset - remainder;
    let mut i = offset;
    while i < end_unroll {
        read_long_be(r, buffer, i, num_hops, num_bytes)?;
        i += num_hops;
    }
    if remainder > 0 {
        read_remaining_longs(buffer, i, r, remainder, num_bytes)?;
    }
    Ok(())
}

fn read_remaining_longs(
    buffer: &mut [i64],
    mut offset: usize,
    r: &mut dyn Read,
    mut remainder: usize,
    num_bytes: usize,
) -> Result<(), Error> {
    let to_read = remainder * num_bytes;
    let mut read_buffer = vec![0u8; to_read];
    r.read_exact(&mut read_buffer)?;

    let mut idx = 0;
    match num_bytes {
        1 => {
            while remainder > 0 {
                buffer[offset] = i64::from(read_buffer[idx] & 255);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        2 => {
            while remainder > 0 {
                buffer[offset] = read_long_be2(&read_buffer, idx * 2);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        3 => {
            while remainder > 0 {
                buffer[offset] = read_long_be3(&read_buffer, idx * 3);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        4 => {
            while remainder > 0 {
                buffer[offset] = read_long_be4(&read_buffer, idx * 4);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        5 => {
            while remainder > 0 {
                buffer[offset] = read_long_be5(&read_buffer, idx * 5);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        6 => {
            while remainder > 0 {
                buffer[offset] = read_long_be6(&read_buffer, idx * 6);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        7 => {
            while remainder > 0 {
                buffer[offset] = read_long_be7(&read_buffer, idx * 7);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        8 => {
            while remainder > 0 {
                buffer[offset] = read_long_be8(&read_buffer, idx * 8);
                offset += 1;
                remainder -= 1;
                idx += 1;
            }
        }
        _ => {
            return Err(Error::new(
                std::io::ErrorKind::InvalidInput,
                "Invalid number of bytes",
            ));
        }
    }

    Ok(())
}

fn read_long_be(
    r: &mut dyn Read,
    buffer: &mut [i64],
    start: usize,
    num_hops: usize,
    num_bytes: usize,
) -> Result<(), Error> {
    let to_read = num_hops * num_bytes;
    let mut read_buffer = vec![0u8; to_read];
    r.read_exact(&mut read_buffer)?;

    match num_bytes {
        1 => {
            for i in 0..8 {
                buffer[start + i] = i64::from(read_buffer[i] & 255);
            }
        }
        2 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be2(&read_buffer, i * 2);
            }
        }
        3 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be3(&read_buffer, i * 3);
            }
        }
        4 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be4(&read_buffer, i * 4);
            }
        }
        5 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be5(&read_buffer, i * 5);
            }
        }
        6 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be6(&read_buffer, i * 6);
            }
        }
        7 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be7(&read_buffer, i * 7);
            }
        }
        8 => {
            for i in 0..8 {
                buffer[start + i] = read_long_be8(&read_buffer, i * 8);
            }
        }
        _ => {
            return Err(Error::new(
                std::io::ErrorKind::InvalidInput,
                "Invalid number of bytes",
            ));
        }
    }

    Ok(())
}

fn read_long_be2(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 0))
}

fn read_long_be3(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 16)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 2] & 255) << 0))
}

fn read_long_be4(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 24)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 16)
        + (i64::from(read_buffer[rb_offset + 2] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 3] & 255) << 0))
}

fn read_long_be5(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 32)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 24)
        + (i64::from(read_buffer[rb_offset + 2] & 255) << 16)
        + (i64::from(read_buffer[rb_offset + 3] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 4] & 255) << 0))
}

fn read_long_be6(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 40)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 32)
        + (i64::from(read_buffer[rb_offset + 2] & 255) << 24)
        + (i64::from(read_buffer[rb_offset + 3] & 255) << 16)
        + (i64::from(read_buffer[rb_offset + 4] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 5] & 255) << 0))
}

fn read_long_be7(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 48)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 40)
        + (i64::from(read_buffer[rb_offset + 2] & 255) << 32)
        + (i64::from(read_buffer[rb_offset + 3] & 255) << 24)
        + (i64::from(read_buffer[rb_offset + 4] & 255) << 16)
        + (i64::from(read_buffer[rb_offset + 5] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 6] & 255) << 0))
}

fn read_long_be8(read_buffer: &[u8], rb_offset: usize) -> i64 {
    ((i64::from(read_buffer[rb_offset] & 255) << 56)
        + (i64::from(read_buffer[rb_offset + 1] & 255) << 48)
        + (i64::from(read_buffer[rb_offset + 2] & 255) << 40)
        + (i64::from(read_buffer[rb_offset + 3] & 255) << 32)
        + (i64::from(read_buffer[rb_offset + 4] & 255) << 24)
        + (i64::from(read_buffer[rb_offset + 5] & 255) << 16)
        + (i64::from(read_buffer[rb_offset + 6] & 255) << 8)
        + (i64::from(read_buffer[rb_offset + 7] & 255) << 0))
}
